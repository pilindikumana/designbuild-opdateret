	...
	const login = async () => {
		...
		try {
			...
			if (response.ok) {
				...
				errorMessage = data.error || 'Login mislykkedes!';
			}
		} catch (error) {
			console.error('Netværksfejl:', error);
			errorMessage = 'Kunne ikke oprette forbindelse til login-serveren.';
		}
	};
</script>
...
>
	...
	>
		...
		{#if errorMessage}
			<div
				class="mb-4 p-3 bg-red-500/20 border border-red-500/40 text-red-800 rounded-lg text-sm font-medium text-center"
			>
				{errorMessage}
			</div>
		{/if}
		...
		<div class="flex flex-col gap-6">
			...
			>
				<label
					...
					for="username-field">Brugernavn</label
				>
			...
			>
				<label
					class="block text-xs uppercase tracking-wider text-slate-600 mb-1"
					for="password-field">Adgangskode</label
				>
				<div class="flex items-center justify-between">
					<input
						id="password-field"
