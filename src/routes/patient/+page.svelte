<script>
	...
	onMount(() => {
		...
		const gemtPatientId = Number(localStorage.getItem('patientId'));
		if (Number.isFinite(gemtPatientId)) {
			patientId = gemtPatientId;
		}
	});
	...
	async function sendToDoctor() {
		...
		try {
			...
			if (response.ok) {
				...
				statusText = resData.error ? `Fejl: ${resData.error}` : 'Fejl: Kunne ikke gemme.';
			}
		} catch (err) {
			console.error(err);
...
<div class="min-h-screen w-full flex items-center justify-center bg-[#bfdbfe] p-4">
	<div
		class="w-90 h-130 bg-black rounded-[45px] border-8 border-[#9c9c9c] shadow-[0_8px_25px_rgba(0,0,0,0.35)] text-white p-6 relative flex flex-col items-center select-none box-border"
	>
		{#if currentPage === 1}
			<div class="w-full flex flex-col items-center pt-6 flex-1">
				...
				<div class="w-full flex justify-around mt-4">
					<div class="flex flex-col items-center">
						...
						<button
							onclick={() => selectDay('Dag 1')}
							class="text-[18px] font-bold my-3 transition-colors outline-none {selectedDay ===
							'Dag 1'
								? 'text-white underline'
								: 'text-zinc-400 hover:text-zinc-200'}">Dag 1</button
						>
						<button
							onclick={() => selectDay('Dag 2')}
							class="text-[18px] font-bold my-3 transition-colors outline-none {selectedDay ===
							'Dag 2'
								? 'text-white underline'
								: 'text-zinc-400 hover:text-zinc-200'}">Dag 2</button
						>
						<button
							onclick={() => selectDay('Dag 3')}
							class="text-[18px] font-bold my-3 transition-colors outline-none {selectedDay ===
							'Dag 3'
								? 'text-white underline'
								: 'text-zinc-400 hover:text-zinc-200'}">Dag 3</button
						>
					</div>
					<div class="flex flex-col items-center">
						<h3 class="underline text-[17px] mb-4 text-zinc-300">Vælg tidspunkt</h3>
						<button
							onclick={() => selectTime('Morgen')}
							class="text-[18px] font-bold my-3 transition-colors outline-none {selectedTime ===
							'Morgen'
								? 'text-white underline'
								: 'text-zinc-400 hover:text-zinc-200'}">Morgen</button
						>
						<button
							onclick={() => selectTime('Aften')}
							class="text-[18px] font-bold my-3 transition-colors outline-none {selectedTime ===
							'Aften'
								? 'text-white underline'
								: 'text-zinc-400 hover:text-zinc-200'}">Aften</button
						>
					</div>
				</div>
				<div class="w-full flex justify-center mt-auto mb-10 min-h-11">
					{#if selectedDay && selectedTime}
						<button
							onclick={startPage}
							class="bg-[#009846] text-white font-bold text-[15px] px-6 py-2.5 rounded uppercase tracking-wide shadow-md hover:bg-[#007a38] transition-colors"
						>
							START MÅLING
						</button>
					{/if}
		...
		{#if currentPage === 2}
			<div class="w-full flex flex-col items-center pt-2 flex-1 relative">
				<h2 class="text-[22px] font-medium leading-tight text-center mt-2">
				...
				{#if !isSent}
					<button
						onclick={goBack}
						class="bg-zinc-800 text-white text-[12px] px-3 py-1 rounded mt-2 self-center hover:bg-zinc-700 transition-colors"
						disabled={isMeasuring}>← Tilbage</button
					>
				{/if}

				<div
					class="w-full grid grid-cols-5 gap-1.5 mt-8 pr-2 pl-1 text-center text-[13px] items-center"
				>
					<div
						class="text-left font-bold text-zinc-300 col-span-5 grid grid-cols-5 gap-1.5 mb-1 text-[11px]"
					>
						<div>1. måling</div>
						<div>2. måling</div>
						<div>3. måling</div>
				...
				<div class="mt-auto mb-4 text-center px-4">
					{#if isSent}
						...
						<div class="text-[13px] text-zinc-200 leading-normal">
							Du kan nu slukke blodtryksmåleren.
						</div>
					{:else}
				...
				<div class="w-full flex justify-center gap-3 mb-10 min-h-10">
					{#if showResultButtons}
						{#if isSent}
							<button
								onclick={goBack}
								class="bg-zinc-800 text-white font-bold text-[14px] px-6 py-2 rounded shadow hover:bg-zinc-700 transition-colors"
							>
								Tilbage til start
							</button>
						{:else}
							<button
								onclick={startPage}
								class="bg-[#b00000] text-white font-bold text-[14px] px-4 py-2 rounded shadow hover:bg-[#8f0000] transition-colors"
							>
								Mål igen
							</button>
							<button
								onclick={sendToDoctor}
								class="bg-[#009846] text-white font-bold text-[14px] px-4 py-2 rounded shadow hover:bg-[#007a38] transition-colors"
							>
								Send til læge
							</button>
						{/if}
		...
		<div
			class="w-11.25 h-11.25 bg-zinc-900 rounded-full absolute bottom-4 left-1/2 -translate-x-1/2 border border-zinc-800"
		></div>
	</div>
</div>
