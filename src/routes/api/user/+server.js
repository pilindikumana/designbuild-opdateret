...
export async function POST({ request }) {
    try {
        ...
        if (!username || !password || !role) {
            return json({ error: 'Alle felter (brugernavn, adgangskode og rolle) skal udfyldes' }, { status: 400 });
        }

        // 2. Hash adgangskoden
        const hashedPass = await bcrypt.hash(password, 10);

        // 3. Indsæt i databasen
        const createdUserArray = await db.insert(user).values({ 
            username, 
            password: hashedPass, 
            role: role 
        }).returning();

        return json({ success: true, user: createdUserArray[0] }, { status: 201 });
        ...
        return json({ error: 'Der opstod en intern fejl i databasen: ' + error.message }, { status: 500 });
    }
}
...
export async function GET() {
    try {
        const patienter = await db.select({
            id: user.id,
            navn: user.username 
        })
        .from(user)
